...
import yearDropdownPlugin from '../../../plugins/yearSelect/index.js'
...
const invalidDateError = ref(null)
...
const parseCustomDateTime = (input) => {
  ...
  invalidDateError.value = ['Invalid date'];
  return null
}
...
compAttrs.config = {
  ...
  plugins: [
      new yearDropdownPlugin({
        ...compAttrs.config,
        date: props.modelValue ? new Date(props.modelValue) : new Date(),
        yearStart: 100,
        yearEnd: 2,
      }),
    ],
  ...
  ...((!['H:i', 'H:i:s', 'H:i a'].includes(compAttrs.config?.dateFormat)) && {
    parseDate: (dateStr) => {
      const parsed = parseCustomDateTime(dateStr)
      return parsed || false
    }
  }),
}
    ...
    >
      <template #default="{ id, isDirty, isValid, isDisabled, isReadonly }">
        ...
        <VField
          ...
          :error="isValid.value === false || invalidDateError?.length > 0"
          :error-messages="invalidDateError"
